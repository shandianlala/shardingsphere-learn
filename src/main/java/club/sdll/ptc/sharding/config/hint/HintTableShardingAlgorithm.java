...
public class HintTableShardingAlgorithm implements HintShardingAlgorithm<String> {
    ...
    public Collection<String> doSharding(Collection<String> availableTargetNames, HintShardingValue<String> shardingValue) {
        log.info("table availableTargetNames={}", JSON.toJSONString(availableTargetNames));
        Collection<String> values = shardingValue.getValues();
        log.info("table values={}", JSON.toJSONString(values));
        String tenantId = values.stream().findFirst().get();
        Optional<String> any = availableTargetNames.stream().filter(tableName -> tableName.endsWith("_" + tenantId)).findAny();
        if (any.isPresent()) {
